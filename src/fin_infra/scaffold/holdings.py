"""
Holdings domain scaffold generation.

This module provides functions to scaffold fin-infra holdings domain code: models, schemas,
repository, and __init__.py. Uses templates from fin_infra.investments.scaffold_templates.

Typical usage:
    from fin_infra.scaffold.holdings import scaffold_holdings_core
    result = scaffold_holdings_core(
        dest_dir=Path("src/my_project/holdings"),
        include_tenant=True,
...
def _generate_substitutions(
    ...
    """
    Generate template substitutions for holdings domain.
    ...
    Args:
        ...
        include_soft_delete: If True, generate deleted_at field patterns (NOTE: Holdings
                            snapshots are mostly immutable, soft delete not commonly used)

    Returns:
        Dict mapping variable names to their substitution values
    """
    subs: Dict[str, str] = {
        "Entity": "HoldingSnapshot",
        "entity": "holding_snapshot",
        "table_name": "holding_snapshots",
    }
    ...
    if include_tenant:
        ...
        subs["tenant_filter"] = ".where(HoldingSnapshot.tenant_id == tenant_id)"
    ...
    # NOTE: Holdings snapshots are mostly immutable, soft delete is unusual
    if include_soft_delete:
        subs["soft_delete_field"] = (
            "\n    deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)"
        )
        subs["soft_delete_filter"] = ".where(HoldingSnapshot.deleted_at.is_(None))"
...
def _generate_init_content(
    ...
    Args:
        models_file: Filename of models file (e.g., "holding_snapshot.py")
        schemas_file: Filename of schemas file (e.g., "holding_snapshot_schemas.py")
    ...
    exports = [
        "HoldingSnapshot",
        "create_holding_snapshot_service",
        "HoldingSnapshotBase",
        "HoldingSnapshotRead",
    ...
    lines = [
        '"""Investment Holdings Snapshot persistence layer (generated by fin-infra scaffold)."""',
        "",
        f"from .{models_module} import HoldingSnapshot, create_holding_snapshot_service",
    ...
    if repo_file:
        ...
        lines.append(f"from .{repo_module} import HoldingSnapshotRepository")
        exports.append("HoldingSnapshotRepository")
...
def scaffold_holdings_core(
    dest_dir: Path,
    include_tenant: bool = False,
    include_soft_delete: bool = False,
    with_repository: bool = True,
    overwrite: bool = False,
    models_filename: str = "holding_snapshot.py",
    schemas_filename: str = "holding_snapshot_schemas.py",
    repository_filename: str = "holding_snapshot_repository.py",
) -> Dict[str, Any]:
    """
    Scaffold holdings domain files: models, schemas, repository (optional), and __init__.py.

    Generates production-ready code from templates in fin_infra.investments.scaffold_templates:
    - models.py.tmpl → HoldingSnapshot model (mostly immutable, only notes can be updated)
    - schemas.py.tmpl → HoldingSnapshotBase, HoldingSnapshotCreate, HoldingSnapshotRead,
                       HoldingSnapshotUpdate (only notes field)
    - repository.py.tmpl → HoldingSnapshotRepository with time-series queries
                          (get_latest, get_by_date, get_by_date_range, get_trend, calculate_performance)
    - README.md → Complete usage guide with snapshot patterns and integration examples

    Args:
        dest_dir: Destination directory (will be created if missing)
        include_tenant: Add tenant_id field for multi-tenancy
        include_soft_delete: Add deleted_at field (NOTE: Unusual for mostly immutable snapshots)
        with_repository: Generate repository file (default True)
        overwrite: Overwrite existing files (default False, skip if present)
        models_filename: Output filename for models (default "holding_snapshot.py")
        schemas_filename: Output filename for schemas (default "holding_snapshot_schemas.py")
        repository_filename: Output filename for repository (default "holding_snapshot_repository.py")
    ...
    Example:
        ...
        >>> from fin_infra.scaffold.holdings import scaffold_holdings_core
        >>> result = scaffold_holdings_core(
        ...     dest_dir=Path("src/my_app/models/holdings"),
        ...     include_tenant=True,
        ...     include_soft_delete=False,
        ...     with_repository=True,
        ...     overwrite=False
        ...
        {
            "files": [
                {"path": "src/my_app/models/holdings/holding_snapshot.py", "action": "wrote"},
                {"path": "src/my_app/models/holdings/holding_snapshot_schemas.py", "action": "wrote"},
                {"path": "src/my_app/models/holdings/holding_snapshot_repository.py", "action": "wrote"},
                {"path": "src/my_app/models/holdings/__init__.py", "action": "wrote"}
            ]
        }
    """
    ...
    # Render templates from fin_infra.investments.scaffold_templates package
    models_content = render_template(
        "fin_infra.investments.scaffold_templates", "models.py.tmpl", subs
    )
    schemas_content = render_template(
        "fin_infra.investments.scaffold_templates", "schemas.py.tmpl", subs
    )
    readme_content = render_template("fin_infra.investments.scaffold_templates", "README.md", subs)

    if with_repository:
        repository_content = render_template(
            "fin_infra.investments.scaffold_templates", "repository.py.tmpl", subs
        )
